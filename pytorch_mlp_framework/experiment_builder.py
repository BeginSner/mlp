...
class ExperimentBuilder(nn.Module):
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        for name, param in named_parameters:
            if(param.requires_grad) and ("bias" not in name): # neglect the bias
                name = name.replace("layer_dict.","").replace(".weight","") # process the str
                layers.append(name)
                all_grads.append(param.grad.abs().mean().detach().cpu().numpy()) # append the gradient
