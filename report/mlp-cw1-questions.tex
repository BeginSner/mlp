...
\youranswer{the use of dropout can reduce the impact of overfitting. When the probability of a neuron to dropout 
increases, the gap between training error and validation error will be smaller. However, the performance of the 
model(we think higher validation accuracy means better performance) gets worse when using dropout. 
Meanwhile, the use of either L1 penalty or L2 penalty has no effect on problem of overfitting, but it will 
severely degrades the performance of model.}
}
...
\begin{table}[t]
    ...
    \midrule
         32            &    0.789   &  0.563 &   0.705            \\
         64            &    0.808   &  0.337  &   0.660            \\
         128           &    0.804   &  0.170  &   0.907           \\ 
    \bottomrule
    \end{tabular}
